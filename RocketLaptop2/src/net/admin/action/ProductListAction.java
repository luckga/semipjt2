...
public class ProductListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getParameter("search_word") == null
			...
			productlist = pdao.getProductList(page, limit);
		}else{ // 검색을 클릭한 경우
			index = Integer.parseInt(request.getParameter("search_field"));
			String[] search_field = new String[] {"PRODUCT_CODE", "PRODUCT_NAME", "CATEGORY_NAME", "PRODUCT_STATUS"};
			search_word = request.getParameter("search_word");
			listcount = pdao.getProductListCount(search_field[index], search_word);
			productlist = pdao.getProductList(search_field[index], search_word, page, limit);
		}
